...
class GameResource < Sinatra::Base
	# Time until token expiration. This should be configurable in the future. 
	...
	put '/games/:token/' do
		...
		if token
			unless token.is_expired?
				...
				# If either update fails (tries to save and can't),
	...
	post '/games/:game_type/user/:user_id/' do
		...
		if game.save && user.save
		    # We don't care if there was a previous token, overwrite it anyway
			token = Token.new
